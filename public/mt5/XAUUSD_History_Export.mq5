//+------------------------------------------------------------------+
//| XAUUSD_History_Export.mq5                                        |
//| Exporta histórico OHLC de XAUUSD a CSV para Trading Compass.     |
//|                                                                  |
//| Cómo usarlo:                                                     |
//|  1. Copiar este archivo en  MQL5/Scripts/  dentro del directorio |
//|     de datos de tu MT5 (File > Open Data Folder).                |
//|  2. Reiniciar MT5 (o click derecho en "Scripts" del Navegador y  |
//|     "Refrescar").                                                |
//|  3. Asegurarse de tener XAUUSD descargado: Tools > History Center|
//|     (Ctrl+U), seleccionar XAUUSD y "Descargar" para cada TF.     |
//|  4. Arrastrar el script sobre cualquier gráfico de XAUUSD.       |
//|  5. Los CSV salen en  MQL5/Files/  (Open Data Folder > MQL5 >    |
//|     Files). Subirlos en la app, sección Backtest.                |
//+------------------------------------------------------------------+
#property script_show_inputs
#property strict

input int     YearsBack = 10;       // Años de histórico a exportar
input string  SymbolName = "XAUUSD";// Símbolo (ajustar si tu broker usa sufijo, ej "XAUUSDm")

void ExportTF(ENUM_TIMEFRAMES tf, string tfName)
{
   datetime to   = TimeCurrent();
   datetime from = to - (datetime)YearsBack * 365 * 24 * 60 * 60;
   MqlRates rates[];
   ArraySetAsSeries(rates, false);
   int copied = CopyRates(SymbolName, tf, from, to, rates);
   if(copied <= 0)
   {
      PrintFormat("Sin datos para %s %s (error=%d)", SymbolName, tfName, GetLastError());
      return;
   }
   string fileName = StringFormat("%s_%s.csv", SymbolName, tfName);
   int fh = FileOpen(fileName, FILE_WRITE|FILE_CSV|FILE_ANSI, ',');
   if(fh == INVALID_HANDLE) { PrintFormat("No se pudo abrir %s", fileName); return; }
   FileWrite(fh, "Date","Open","High","Low","Close","Volume");
   for(int i = 0; i < copied; i++)
   {
      MqlDateTime mdt;
      TimeToStruct(rates[i].time, mdt);
      string dateStr = StringFormat("%04d.%02d.%02d %02d:%02d",
                                    mdt.year, mdt.mon, mdt.day, mdt.hour, mdt.min);
      FileWrite(fh,
                dateStr,
                DoubleToString(rates[i].open,  2),
                DoubleToString(rates[i].high,  2),
                DoubleToString(rates[i].low,   2),
                DoubleToString(rates[i].close, 2),
                (long)rates[i].tick_volume);
   }
   FileClose(fh);
   PrintFormat("Exportadas %d velas → %s", copied, fileName);
}

void OnStart()
{
   PrintFormat("Exportando histórico de %s (%d años)...", SymbolName, YearsBack);
   ExportTF(PERIOD_D1,  "D1");
   ExportTF(PERIOD_H4,  "H4");
   ExportTF(PERIOD_H1,  "H1");
   ExportTF(PERIOD_M15, "M15");
   ExportTF(PERIOD_M5,  "M5");
   ExportTF(PERIOD_M1,  "M1");
   Print("Listo. Archivos CSV en MQL5/Files/");
}